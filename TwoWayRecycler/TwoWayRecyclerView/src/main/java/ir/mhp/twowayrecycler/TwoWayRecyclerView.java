...
import ir.mhp.twowayrecycler.swipe.SwipeDismissRecyclerViewTouchListener;
import ir.mhp.twowayrecycler.util.FloatUtil;

@SuppressLint("NewApi")
public class TwoWayRecyclerView extends FrameLayout {

    protected int ITEM_LEFT_TO_LOAD_MORE = 10;
    protected int ITEM_LEFT_TO_REACH_TOP = 10;
    ...
    private boolean isReachedRealTop = false;
    private boolean isReachedRealBottom = false;

    public void setReachedRealTop(boolean reachedRealTop) {
        isReachedRealTop = reachedRealTop;
    }

    public void setReachedRealBottom(boolean reachedRealBottom) {
        isReachedRealBottom = reachedRealBottom;
    }

    public int getITEM_LEFT_TO_LOAD_MORE() {
        return ITEM_LEFT_TO_LOAD_MORE;
    }

    public boolean isReachedRealTop() {
        return isReachedRealTop;
    }

    public boolean isReachedRealBottom() {
        return isReachedRealBottom;
    }
    ...
    public TwoWayRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttrs(attrs);
        initView();
    }

    public TwoWayRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private int getFirstVisibleItemPosition(RecyclerView.LayoutManager layoutManager) {
        ...
        switch (layoutManagerType) {
            ...
            case GRID:
                ...
                break;
            case STAGGERED_GRID:
                firstVisibleItemPosition = caseStaggeredGridFirstItem(layoutManager);
                break;
        }
    ...
    private int caseStaggeredGridFirstItem(RecyclerView.LayoutManager layoutManager) {
        StaggeredGridLayoutManager staggeredGridLayoutManager = (StaggeredGridLayoutManager) layoutManager;
        if (lastScrollPositions == null)
            lastScrollPositions = new int[staggeredGridLayoutManager.getSpanCount()];
     ...
     *
     * @param numToLoadMore Number of items before loading more
     * @param numToReachTop Number of items before reach top
     */
    public void setupMoreListener(OnMoreListener onMoreListener, int numToLoadMore, int numToReachTop) {
        mOnMoreListener = onMoreListener;
        ITEM_LEFT_TO_LOAD_MORE = numToLoadMore;
        ITEM_LEFT_TO_REACH_TOP = numToReachTop;
    }
    ...
    public void setNumberBeforeReachTopIsCalled(int max) {
        ITEM_LEFT_TO_REACH_TOP = max;
    }
